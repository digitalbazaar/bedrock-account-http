...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
       ...
       * @param {string} req.body.action - Action type: create, update.
       * @param {object} req.body.productIds - Product IDs to create a meters.
       * @param {string} req.user.account.id - User account ID.
       * @description Looks up a the request user's account ID in a list of
       * meter creation policies and returns a list of products that to perform
       * a meter operation for ('create', 'update', etc).
       */
      ...
      const availableProducts = accountPolicies[action];

      // check for account meter creation policy
      if(!Object.keys(availableProducts).length) {
        throw new BedrockError(
          'Missing meter creation policy for account.', 'NotFoundError', {
            httpStatusCode: 404,
            public: true,
          });
      }

      // ensure all product IDs are listed in policy & creation is permitted
      productIds.forEach(id => {
        const product = Object.keys(availableProducts).find(productId => {
          return productId === id && availableProducts[productId].allowed;
        });
        if(!product) {
          throw new BedrockError(
            `Policy does not include product ID: ${id}`, 'NotFoundError', {
              httpStatusCode: 404,
              public: true,
            });
        }
      ...
      const {metersByProductIds} = await HANDLERS.createMeter({productIds});

      return res.status(200).json({metersByProductIds});
    }));
