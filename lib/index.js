...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(cursor) {
        query["account.created"] = {$gte: cursor};
      }
      const options = {limit, sort: {"account.created": -1}};
  ...
  app.patch(
    ...
    asyncHandler(async (req, res, next) => {
       ...
       * patches need to be the [json patch format]{@link https://www.npmjs.com/package/fast-json-patch}
       */
      try {
        const {id, patch, sequence} = req.body;   
        const {actor} = req.user || {};
        const updater = {
          actor,
          ...req.body
        };
        const update = await brAccount.update(updater);
        res.status(200).send();
      } catch(e) {
        console.error(e);
